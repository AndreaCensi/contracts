...
class Extension(Contract):
    ...
    def check_contract(self, context, value, silent):
        ...
        context["kwargs"] = dict((k, v.eval(context)) for k, v in self.kwargs.items())
    ...
    def __getstate__(self):
        return {"identifier": self.identifier, "args": self.args, "kwargs": self.kwargs}
...
def build_args_kwargs(s, loc, tokens):
    return (
        tuple(t for t in tokens if not isinstance(t, dict)),
        dict((k, v) for t in tokens if isinstance(t, dict) for k, v in t.items()),
    )
