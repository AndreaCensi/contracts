...
class DocStringInfo(object):
    ...
    def parse(docstring):
        ...
        docstring, params_ann = parse_annotations(docstring, param_keys, False, True)
        docstring, types_ann = parse_annotations(docstring, type_keys, False, False)
        docstring, returns_ann = parse_annotations(docstring, return_keys, True, True)
        docstring, rtype_ann = parse_annotations(docstring, rtype_keys, True, False)

        params = {}
        names = set(list(params_ann.keys()) + list(types_ann.keys()))
        for name in names:
            param_type, param_desc = params_ann.get(name, (None, None))
            params[name] = Arg(param_desc, param_type or types_ann.get(name, None))

        returns = []
        for i in range(max(len(returns_ann), len(rtype_ann))):
            return_type, return_desc = returns_ann.get(i, (None, None))
            returns.append(Arg(return_desc, return_type or rtype_ann.get(i, None)))
...
def parse_annotations(docstring, keys, empty=False, inline_type=False):
    ...
    for key in keys:
        if empty:
            regexp = r"^\s*:\s*%s(?P<type>[^:]*?)\s*:\s*(?P<desc>.*?)\s*$" % key
        else:
            regexp = r"^\s*:\s*%s\s+(?P<type>[^:]*?)(?P<name>[^\s:]+)\s*:\s*(?P<desc>.*?)\s*$" % key
